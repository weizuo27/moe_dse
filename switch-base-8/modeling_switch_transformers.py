...
class SwitchTransformersDenseActDense(nn.Module):
    ...
    def __init__(self, config: SwitchTransformersConfig):
        ...
        self.saved = SwitchTransformersDenseActDense.saved
        SwitchTransformersDenseActDense.saved += 1

    def forward(self, hidden_states):
        #if self.saved == 1:
        #    torch.save(hidden_states, "hidden_states_before.pt")

        hidden_states = self.wi(hidden_states)
        hidden_states = self.act(hidden_states)
        hidden_states_copy = torch.clone(hidden_states.detach())
        hidden_states = self.dropout(hidden_states)
        assert(torch.allclose(hidden_states, hidden_states_copy))
        #if self.saved == 1:
        #    torch.save(hidden_states_copy, "hidden_states_copy.pt")
...
class SwitchTransformersSparseMLP(nn.Module):
    ...
    def __init__(self, config: SwitchTransformersConfig, expert_class: nn.Module = SwitchTransformersDenseActDense, layer_id=-1):
        super().__init__()
    ...
    def forward(self, hidden_states):
        ...
        router_mask_sum = router_mask.sum((0, 1))
        #uncomment part1 for expert token distribution analysis
        #part1: begin
        #print("layer_id ", self.layer_id, router_mask.shape, router_mask_sum)
        #part1: end
        ...
        #torch.save(hidden_states, f"hidden_states_{self.layer_id}.pt")
        #torch.save(router_mask, f"router_mask_{self.layer_id}.pt")

        #uncomment part2 and part3 for execution time collection
        #part2: begin
        #if self.fwd_times <1 :
        #    start = time.perf_counter()
        #part2: end
        for idx, expert in enumerate(self.experts.values()):
            #wi_p = [p for p in expert.wi.parameters()]
            #wo_p = [p for p in expert.wo.parameters()]
            #assert(len(wi_p) == len(wo_p) == 1)
            #torch.save(wi_p[0].data.transpose(1, 0).flatten(), f"wi_layer{self.layer_id}_exp{idx}.pt")
            #torch.save(wo_p[0].data.transpose(1, 0).flatten(), f"wo_layer{self.layer_id}_exp{idx}.pt")
        ...
        #    print("dur_time", end, start, self.fwd_times, self.layer_id, dur_time)
        #    import sys; sys.stdout.flush();
        #part3: end

        self.fwd_times += 1
        #torch.save(next_states, f"next_states_{self.layer_id}.pt")
...
class SwitchTransformersLayerFF(nn.Module):
    ...
    def __init__(self, config: SwitchTransformersConfig, is_sparse=False, layer_id = -1):
        super().__init__()
        ...
        if not self.is_sparse:
            ...
            self.mlp = SwitchTransformersSparseMLP(config, layer_id = layer_id)
...
class SwitchTransformersBlock(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, is_sparse=False, layer_id = -1):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.is_sparse = is_sparse
        ...
        self.layer.append(SwitchTransformersLayerFF(config, is_sparse=self.is_sparse, layer_id = layer_id))
...
class SwitchTransformersStack(SwitchTransformersPreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        for i in range(config.num_layers):
            ...
            self.block.append(
                SwitchTransformersBlock(config, has_relative_attention_bias=bool(i == 0), is_sparse=is_sparse, layer_id = i)
            )
...
class SwitchTransformersModel(SwitchTransformersPreTrainedModel):
    ...
    def forward(
        ...
        #print("encoder_outputs", encoder_outputs.shape, hidden_states.shape)
